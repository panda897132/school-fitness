...
HRESULT DLLAPI PathCchIsRoot(const WCHAR *path);
...
HRESULT DLLAPI PathCchIsRoot(const WCHAR *path)
{
	...
	if ((is_unc = is_prefixed_unc(path)) || (path[0] == '\\' && path[1] == '\\' && path[2] != '?'))
	{
		...
		else
		{
			...
			return !get_next_segment(next, &next) && !*next ? S_OK : S_FALSE;
		}
	}
...
HRESULT DLLAPI PathCchRemoveFileSpec(WCHAR *path, SIZE_T size)
{
	...
	if (PathCchIsRoot(path) == S_OK) return S_FALSE;
